...
function renderFunnel(){
  ...
  ${state.scope==='ALL'&&!DATA.attribution.india_reference_status?`<div class="warn"><b>Reading the combined view:</b> India FY2025 is loaded from the complete CSV and attributes its held-back rows at <code>Final routing</code> (Unmapped / manufacturer-only) rather than at Reference validation, because that source lacks reference-status columns. Per-file views are cleaner — switch the market selector above.</div>`:''}`;
}
...
function renderSimulator(){
  ...
  <div class="card"><h3>${DATA.attribution.india_reference_status?'Locked terminal outcomes':'Locked coverage and attribution blocks'}</h3><p><b>S13 Final routing is shown but cannot be toggled.</b> ${DATA.attribution.india_reference_status?'These rows are non-toggleable terminal outcomes, predominantly genuine coverage gaps such as Unmapped or manufacturer-only records, rather than one production gate that can honestly be switched off.':'These rows are coverage gaps or an audit-source attribution artifact, not a single production gate that can honestly be switched off.'}</p><div class="numgrid"><span>Locked here: <b>${fmtM(locked)}</b></span></div>${lockedReasons.length?`<table><thead><tr><th>Reason</th><th>Rows</th><th>Value</th></tr></thead><tbody>${lockedReasons.map(x=>`<tr><td>${esc(prettyReason(x.reason))}</td><td>${fmtInt(x.m[0])}</td><td>${fmtMoney(x.m[1])}</td></tr>`).join('')}</tbody></table>`:''}${DATA.attribution.india_reference_status?'<div class="info small"><b>India FY2025 attribution fixed:</b> governed master-validation evidence is carried into this audit, so its non-Trusted reference losses now appear at S07 Reference validation.</div>':'<div class="warn small"><b>India FY2025 caveat:</b> its audit source lacks reference-status columns, so many losses appear at S13 rather than S07. They are visible here but intentionally locked until the next audit carries those fields.</div>'}</div>
...
function render(){
  ...
  document.getElementById('scopebanner').innerHTML = state.scope==='ALL'&&!DATA.attribution.india_reference_status
